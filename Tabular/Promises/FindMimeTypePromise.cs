...
{
    public class FindMindTypePromise: Promise<OpenTabularFileWorkload>
    {
        ...
        private string GetMimeType(string sFilePath)
        {
            string sMimeType = GetMimeTypeFromList(sFilePath);

            if (!String.IsNullOrEmpty(sMimeType)) return sMimeType;
        ...
        private static string GetMimeTypeFromList(string sFileNameOrPath)
        {
            string sMimeType = null;
            var extension = Path.GetExtension(sFileNameOrPath);
            if (extension == null) return null;
            var sExtensionWithoutDot = extension.Substring(1).ToLower();
        ...
        private static string GetMimeTypeFromRegistry(string sFileNameOrPath)
        {
            string sMimeType = null;
            var extension = Path.GetExtension(sFileNameOrPath);
            ...
            var sExtension = extension.ToLower();
            var pKey = Registry.ClassesRoot.OpenSubKey(sExtension);
        ...
        private string GetMimeTypeFromFile(string sFilePath)
        {
            string sMimeType = null;

            if (!File.Exists(sFilePath)) return null;

            var abytBuffer = new byte[256];

            using (var pFileStream = new FileStream(sFilePath, FileMode.Open))
            {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(sMimeTypeFromFile) && sMimeTypeFromFile != "text/plain" &&
                    sMimeTypeFromFile != "application/octet-stream")
                {
